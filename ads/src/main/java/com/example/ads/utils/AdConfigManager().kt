...
fun Activity.homeNativeSlider(): AdConfigModel? {
    return try {
        val native = newAdsConfig?.homeScreen
        native?.let {
            AdConfigModel(
                idHigh = it.adUnitIds?.getOrNull(0) ?: getString(R.string.native_exit_low),
                idMedium = it.adUnitIds?.getOrNull(1) ?: getString(R.string.native_exit_low),
                idBackUp = it.adUnitIds?.getOrNull(2) ?: getString(R.string.native_exit_low),
                reloadLimit = it.reloadLimit ?: 2,
                whichAd = AdsClassification.NATIVE,
                isAboveCtr = false,
                isMetaLayout = false,
                ...
                currentActivityOrFragment = "HOME_NATIVE_SLIDER"
            )
        }
    } catch (ex: Exception) {
        null
...
fun nativeSplash(
    idHigh: String = "ca-app-pub-3940256099942544/2247696110",
    idMedium: String = "ca-app-pub-3940256099942544/2247696110",
    idBackUp: String = "ca-app-pub-3940256099942544/2247696110"
): AdConfigModel? {
    return try {
        AdConfigModel(
            idHigh = idHigh,
